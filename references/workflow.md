...
## 阶段 10：导师/评委意见定点修订（收到反馈后启动）

完整流程见 `revision-workflow.md`。要点：意见解码到病灶节号 → 分清"评分标准类"与"写作范式类"参考文献 → 修改矩阵 → **归因实验先行锁定新数值** → 三类标准动作（模型显式化/逻辑链显式化/精简打磨）→ 验收 + 修订对照表。每轮成稿存 versions/ 快照（tex+pdf 成对）。
